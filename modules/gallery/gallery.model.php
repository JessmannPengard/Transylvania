...
class Gallery
{
    ...
    {
        $stm = $this->dbconn->prepare("SELECT * FROM gallery WHERE id = :id");
    ...
    {
        $stm = $this->dbconn->query("SELECT COUNT(*) AS total FROM gallery");
    ...
    {
        $stm = $this->dbconn->prepare("INSERT INTO gallery (id_user, url_picture) VALUES (:id_user, :url_picture)");
    ...
    {
        $stm = $this->dbconn->prepare("DELETE FROM gallery WHERE id=:id");
        $stm->bindValue(":id", $id_imagen);
        $stm->execute();
    }
    ...
    public function getImages($page, $perPage)
    {
        ...
        $sql = "SELECT id, url_picture, id_user FROM gallery ORDER BY date DESC LIMIT :limit OFFSET :offset";
